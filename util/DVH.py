...
class DVH(object):
    ...
    def __init__(self,
                 ...
                 # 项目的结果路径
                 root_path="/home/zhaosheng/paper2/online_code/cbamunet-pix2pix/results/",
        ...
        img = ants.image_read(os.path.join(self.root_path,f"real_ct_niis_0318/{pname}.nii"))
        self.raw_img = img
        
        
        self.ct = img.numpy().swapaxes(0,1)
    ...
    def _get_dose_array_from_csv(self,outpath,pname):
        ...
        self.boron_array_real = self._array_from_MC(os.path.join(outpath,"real",pname,"boron10.csv"))
        self.fast_array_real = self._array_from_MC(os.path.join(outpath,"real",pname,"fast10.csv"))
        self.gamma_array_real = self._array_from_MC(os.path.join(outpath,"real",pname,"gamma10.csv"))
        self.nitrogen_array_real = self._array_from_MC(os.path.join(outpath,"real",pname,"nitrogen10.csv"))
        self.boron_array_fake = self._array_from_MC(os.path.join(outpath,"fake",pname,"boron10.csv"))
        self.fast_array_fake = self._array_from_MC(os.path.join(outpath,"fake",pname,"fast10.csv"))
        self.gamma_array_fake = self._array_from_MC(os.path.join(outpath,"fake",pname,"gamma10.csv"))
        self.nitrogen_array_fake = self._array_from_MC(os.path.join(outpath,"fake",pname,"nitrogen10.csv"))
    ...
    def plot_dvh(self,save_fig_path="none",show_pic=False):
        ...
        fg=plt.figure(figsize=(5,5),dpi=400)
