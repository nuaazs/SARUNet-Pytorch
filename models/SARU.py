...
class SARU(nn.Module):
    def __init__(self, input_nc=1, output_nc=1, num_downs=5, ngf=64,  addtional_n=0):#norm_layer=nn.BatchNorm2d,
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            up = [uprelu, upconv, nn.BatchNorm2d(outer_nc), uprelu, upDS, nn.BatchNorm2d(outer_nc)]
            model = down + up
        else:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, bias=use_bias)
            down = [downrelu, downDS, nn.BatchNorm2d(inner_nc)]
            up = [uprelu, upconv, nn.BatchNorm2d(outer_nc), uprelu, upDS, nn.BatchNorm2d(outer_nc)]
            model = down + [submodule] + up
