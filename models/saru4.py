...
class SARU4(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(SARU4, self).__init__()
        ...
        self.down_plus = Down(1024 // factor, 1024 // factor)
        self.up_plus = Up(1024, 512, bilinear)
    ...
    def forward(self, x):
        ...
        x4 = self.down3(x3)
        x5 = self.down4(x4)
        x6 = self.down_plus(x5)
        x7 = self.down_plus(x6)

        x = self.up_plus(x7, x6)
        x = self.up_plus(x, x5)
        x = self.up1(x, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
