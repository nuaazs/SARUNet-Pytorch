...
from models_v2.ssn_cls import SSN_CLS
from models_v2.unet import UNet
from models_v2.denseunet import DenseUnet
from models_v2.resunet import ResUNet
from models_v2.se_resunet import SeResUNet, Shallow_SeResUNet
from models_v2.deeplab_v3p import DeepLab
from models_v2.resnet18 import Res18
from models_v2.cbam_resunet import CbamResUNet
from models_v2.dilated_resunet import DilatedResUnet
from models_v2.vggunet import VGGUNet
...
class UpDS(nn.Module):
    ...
    def forward(self, x): # x1, x2
        x1 = torch.split(x,2,1)[0]
        x2 = torch.split(x,2,1)[1]
        x1 = self.up(x1)
        # input is CHW
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=True, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'attnunet':  #4.9M
        ...
        net = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout,use_depthwise=True,use_attn=True)
    elif netG == 'attnunet_256_ds':
        net = UnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout,use_depthwise=True,use_attn=True)
    elif netG == 'unet_128':
        ...
        net = UnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout,use_depthwise=False,use_attn=False)
    elif netG == 'unet_128_ds':
        net = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout,use_depthwise=True,use_attn=False)
    ...
    #    net = StyleGAN(input_nc,output_nc)
        ...
        net = UNet(input_nc, output_nc, deep_supervision=deep_supervision)
    # elif netG == 'r2attunet':
    #     net = R2AttU_Net(input_nc, output_nc)
    elif netG == 'resunetv2':
        net = ResUNet(input_nc, output_nc, deep_supervision=deep_supervision)
    elif netG == 'se_resunet':
        net = SeResUNet(input_nc, output_nc, deep_supervision=deep_supervision, dropout=use_dropout, rate=0.1)
        # net = Shallow_SeResUNet(input_nc, output_nc, deep_supervision=deep_supervision, dropout=dropout, rate=0.1)
    # elif netG == 'ssn':
    #    net = SSN(output_nc, args.context_path, input_nc=input_nc)
    # elif 'vgg' in netG:
    #     encoder = netG[:-5]
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False,use_depthwise=True,use_attn=True):
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True,depthwise=use_depthwise,attn=use_attn)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout,depthwise=use_depthwise,attn=use_attn)
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer,depthwise=use_depthwise,attn=use_attn)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False,depthwise=False,attn=True):
        ...
        upDS = UpDS(inner_nc * 2, outer_nc, True, kernels_per_layer=4)

        downrelu = nn.LeakyReLU(0.2, True)
        downnorm = norm_layer(inner_nc)
        uprelu = nn.ReLU(True)
        upnorm = norm_layer(outer_nc)
        cbam = CBAM(input_nc,reduction_ratio=2)
        ...
        if outermost:
            ...
            if attn:
                if depthwise:
                    down = [cbam,  downrelu, downDS, downnorm] # downconv
                    up = [uprelu,upconv, upnorm] #  upconv
                else:
                    down = [cbam,  downrelu, downconv, downnorm] # downconv
                    up = [uprelu,upconv, upnorm] #  upconv
                ...
                if depthwise:
                    ...
                    down = [downrelu, downconv, downnorm] # downconv
                    up = [uprelu,upconv, upnorm] #  upconv
