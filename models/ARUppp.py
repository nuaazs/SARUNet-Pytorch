...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, output_channels, kernel_size, padding=0, kernels_per_layer=1):
        ...
        # self.conv = nn.Conv2d(in_channels, in_channels * kernels_per_layer,kernel_size=kernel_size,stride=2, padding=1, bias=True)
...
class DoubleConvDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, kernels_per_layer=1):
        ...
        self.sigle_conv = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0))
    def forward(self, x):
        x1 = self.double_conv(x)
        x2 = self.sigle_conv(x)
        x3 = x1 + x2
        return self.re_lu(x3)
